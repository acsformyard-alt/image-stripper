...
export function uiInit({ inpaintBatch, statusEl }) {
  ...
  async function handleProcess() {
    ...
    if (!files || files.length === 0) {
      _statusEl.textContent = 'Choose images first.';
      return;
    }
    ...
    for (let i = 0; i < bitmaps.length; i++) {
      ...
      const res = await _inpaintBatch([bitmaps[i].bmp]); // single at a time
      outs.push(res[0]);
    }
    ...
    outs.forEach((outCanvas, i) => {
      ...
      canv.width = outCanvas.width;
      canv.height = outCanvas.height;
