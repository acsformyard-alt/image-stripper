...
// Fixed-prompt: mask the upper-right rectangle and inpaint.
...
/** Initialize from URL (works on http/https; often blocked on file://). */
export async function initLamaFromUrl(modelUrl, executionProviders = _executionProviders) {
...
/** Initialize from bytes (works everywhere, including file://). */
...
// Tune this if your corner stamp is larger/smaller
const UPPER_RIGHT_FRACTION = { w: 0.28, h: 0.24 }; // 28% width × 24% height

export async function inpaintUpperRightBatch(bitmaps) {
  if (!session) throw new Error('Model not initialized. Pick or auto-load the .onnx first.');
  const outs = [];
...
async function runLamaInpaint(srcCanvas, maskCanvas) {
  // Preprocess: letterbox to 512x512 (adjust if your ONNX expects a different size)
  const target = 512;
  ...
  // Adjust input names if your model differs
...
function buildUpperRightMask(canvas, frac) {
  ...
  const ry = 0;
  g.fillStyle = 'rgba(255,255,255,1)'; // white = masked
...
function nchwToCanvas(data, H, W) {
  ...
  for (let y=0; y<H; y++) {
    for (let x=0; x<W; x++) {
      const o = y*W + x;
      const r = clamp01(data[0*plane + o]);
      const gg= clamp01(data[1*plane + o]);
      const b = clamp01(data[2*plane + o]);
      const i = o*4;
      img.data[i  ] = Math.round(r*255);
      img.data[i+1] = Math.round(gg*255);
